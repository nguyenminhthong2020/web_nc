...
export default class TransferForm extends React.Component {
    ...
    onKeyDown = async (e) => {
        if (e.key === 'Enter') { // Call axios
            ...
            let receiverInfo = {
                ...
                bankCode: document.getElementById('selectBank').value
            };
            ...
            if (receiverInfo.bankCode == '25Bank') { // Đối tác RSA
                ...
                const response = await connector.post(`/list-receiver1/partner/find`, reqBody).then((response) => {
            ...
            if (receiverInfo.bankCode == 'partner34') { // Đối tác PGP
                ...
                const response = await connector.post(`/list-receiver1/partner/find-pgp`, reqBody).then((response) => {
                    console.log("response", response);
                    if (response.data.status == 'OK') {
                        nameReceiver = response.data.fullname;
                    }
    ...
    selectBankChange(e) {
        ...
        // Set state
        this.setState({bank_code: res.bankCode, numberReceiver: '', nameReceiver: ''})
    }
    ...
    selectReceiverChange = async (e) => {
        ...
        if (receiverInfo.bankCode == '25Bank') {
            // Đối tác RSA
            // Cập nhật giá trị select Bank
            document.getElementById('selectBank').value = '25Bank';
            const reqBody = {
                receiver_account_number: receiverInfo.number
            }
            const response = await connector.post(`/list-receiver1/partner/find`, reqBody).then((response) => {
        ...
        if (receiverInfo.bankCode == 'partner34') {
            // Đối tác PGP
            // Cập nhật giá trị select Bank
            document.getElementById('selectBank').value = 'partner34';
            const reqBody = {
                receiver_account_number: receiverInfo.number
            }
            const response = await connector.post(`/list-receiver1/partner/find-pgp`, reqBody).then((response) => {
    ...
    submitForm = async (e) => {
        ...
        if (isValid) { // Call axios
            const reqBody = {
                ...
                receive_bank_code: document.getElementById('selectBank').value,
                money: this.state.money,
                message: this.state.message,
                type_fee: this.state.method == 1 ? "1" : "0"
            }
            const response = await connector.post(`/transfer/external/receiver_account_number`, reqBody).then((response) => {
                console.log("response", response);
                if (response.data.status == 'OK') { // Lưu vào state
                    const bank_code = reqBody.receive_bank_code;
                    this.setState({activeTab: 1, otp_id: response.data.data.otp_id, email: response.data.data.email, bank_code: bank_code});
    ...
    confirmForm = async (e) => {
        ...
        if (1) { // Call axios
            ...
            const response = await connector.post(`/transfer/external/confirm`, reqBody).then((response) => {
                console.log("response", response);
                if (response.data.status == 'OK') { // Chuyển sang màn hình giao dịch thành công
                            ...
                            <Form onSubmit={
                                ...
                                <Card>
                                    ...
                                    <CardBody>
                                        ...
                                        <FormGroup row>
                                            <Col md="3" className="d-flex p-3">
                                                <Label htmlFor="selectBank">Ngân hàng</Label>
                                            </Col>
                                            <Col xs="12" md="6">
